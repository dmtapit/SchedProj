...
/*
 Called by the system of wxWidgets when the panel needs to be redrawn.
...
/*
 ...
 it is possible that e.g. the window manager throws away your drawing when
 the window comes to the background, and expects you will redraw it when the
 window comes back (by sending a paint event).
...
/*
 Here we do the actual rendering. I put it in a separate method so that it can
 work no matter what type of DC (e.g. wxPaintDC or wxClientDC) is used.
 */
